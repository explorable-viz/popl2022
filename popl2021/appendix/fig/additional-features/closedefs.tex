\begin{figure}
\flushleft\shadebox{$\rho, \delta \closeDefs \rho'$}
\begin{salign}
   \rho, \delta
   &\closeDefs
   [f: \exClosureRecMutual{\rho}{\delta}{\exLambda{\sigma}} \mid \exFun{f}{\sigma} \in \delta]
\end{salign}
\flushleft\shadebox{$\rho, \delta, \alpha \closeDefsFwd \rho'$}
\begin{salign}
   \rho, \delta, \alpha
   &\closeDefsFwd
   [f : \annot{\exClosureRecMutual{\rho}{\delta}{\exLambda{\sigma}}}{\alpha} \mid \exFun{f}{\sigma} \in \delta]
\end{salign}
\flushleft\shadebox{$\rho \closeDefsBwd \rho', \delta, \alpha$}
\begin{salign}
   [f : \annot{\exClosureRecMutual{\rho_f}{\delta_f}{\exLambda{\sigma_f}}}{\alpha_f} \mid f \in F]
   &\closeDefsFwd
   {\textstyle{\bigvee}\rho}, [
   \exFun{f}{\sigma_f} \mid f \in F] \join {\textstyle{\bigvee}\delta},
   {\textstyle{\bigvee}\alpha}
\end{salign}
\caption{Mutually recursive functions $\delta$ close to $\rho'$ in $\rho$}
\end{figure}
