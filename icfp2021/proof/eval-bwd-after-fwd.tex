\subsection{\thmref{core-language:eval:gc}, part (ii)}
Induction on the $\evalFwdS$ derivation.
\begin{proof}

\small
\begin{flalign}
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalFwdS$-var}}
      ]
      {
         e \eq \exVar{x}
         \\
         \envLookup{\rho}{x}{v}
      }
      {
         \evalFwd{\rho}{e}{\alpha}{\trVar{x}{\Gamma}}{v}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \rho \geq
   \envLookupBwd{\rho'}{\Gamma}{\bind{x}{v}}
   \quad
   (\exists \rho')
   &
   \text{
   	(totality; \lemref{core-language:env-get-put})
   }
   \notag
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-var}}
      ]
      {
         \envLookupBwd{\rho'}{\Gamma}{\bind{x}{v}}
      }
      {
         \evalBwd{v}{\trVar{x}{\Gamma}}{\rho'}{\exVar{x}}{\FF}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   (\rho', \exVar{x}, \FF) \leq (\rho, e, \alpha)
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalFwdS$-op}}
      ]
      {
         e \eq \exOp{\primOp}
         \\
         \envLookup{\rho}{\primOp}{v}
      }
      {
         \evalFwd{\rho}{e}{\alpha}{\trOp{\primOp}{\Gamma}}{v}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \rho \geq
   \envLookupBwd{\rho'}{\Gamma}{\bind{\primOp}{v}}
   \quad
   (\exists \rho')
   &
   \text{
   	(totality; \lemref{core-language:env-get-put})
   }
   \notag
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-op}}
      ]
      {
         \envLookupBwd{\rho'}{\Gamma}{\bind{\primOp}{v}}
      }
      {
         \evalBwd{v}{\trOp{\primOp}{\Gamma}}{\rho'}{\exOp{\primOp}}{\FF}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   (\rho', \exOp{\primOp}, \FF) \leq (\rho, e, \alpha)
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[lab={\ruleName{$\evalFwdS$-lambda}}]
      {
         e \eq \exLambda{\sigma}
      }
      {
         \evalFwd{\rho}{e}{\alpha}{\trLambda{\sigma'}}{\exClosure{\rho}{\seqEmpty}{\sigma}}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-lambda}}
      ]
      {
         \strut
      }
      {
         \evalBwd{\exClosure{\rho}{\seqEmpty}{\sigma}}{\trLambda{\sigma'}}{\rho}{\exLambda{\sigma}}{\FF}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   (\rho, \exLambda{\sigma}, \FF) \leq (\rho, e, \alpha)
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[lab={\ruleName{$\evalFwdS$-int}}]
      {
         e \eq \annInt{n}{\alpha}
      }
      {
         \evalFwd{\rho}{e}{\alpha'}{\trInt{n}{\Gamma}}{\annInt{n}{\alpha \meet \alpha'}}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-int}}
      ]
      {
         \strut
      }
      {
         \evalBwd{\annInt{n}{\alpha \meet \alpha'}}
                 {\trInt{n}{\Gamma}}{\hole_{\Gamma}}
                 {\annInt{n}{\alpha \meet \alpha'}}
                 {\alpha \meet \alpha'}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   (\hole_{\Gamma}, \annInt{n}{\alpha \meet \alpha'}, \alpha \meet \alpha')
   \leq
   (\rho, e, \alpha)
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[lab={\ruleName{$\evalFwdS$-nil}}]
      {
         e \eq \annNil{\alpha}
      }
      {
         \evalFwd{\rho}{e}{\alpha'}{\trNil{\Gamma}}{\annNil{\alpha \meet \alpha'}}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-nil}}
      ]
      {
         \strut
      }
      {
         \evalBwd{\annNil{\alpha \meet \alpha'}}
                 {\trNil{\Gamma}}
                 {\hole_{\Gamma}}
                 {\annNil{\alpha \meet \alpha'}}
                 {\alpha \meet \alpha'}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \qedLocal
   (\hole_{\Gamma}, \annNil{\alpha \meet \alpha'}, \alpha \meet \alpha')
   \leq
   (\rho, e, \alpha)
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalFwdS$-cons}},
      ]
      {
         e \eq \annCons{e_1}{e_2}{\alpha}
         \\
         \evalFwd{\rho}{e_1}{\alpha'}{T}{v_1}
         \\
         \evalFwd{\rho}{e_2}{\alpha'}{U}{v_2}
      }
      {
         \evalFwd{\rho}{e}{\alpha'}{\trCons{T}{U}}{\annCons{v_1}{v_2}{\alpha \meet \alpha'}}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \evalBwd{v_1}{T}{\rho_1}{e_1'}{\beta}
   \leq
   \rho, e_1, \alpha'
   \quad
   (\exists \rho_1, e_1', \beta)
   &
   \text{IH}
   \locallabel{cons-premise-one}
   \\
   &
   \evalBwd{v_2}{U}{\rho_2}{e_2'}{\beta'}
   \leq
   \rho, e_2, \alpha'
   \quad
   (\exists \rho_2, e_2', \beta')
   &
   \text{IH}
   \locallabel{cons-premise-two}
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-cons}}
      ]
      {
         \evalBwd{v_1}{T}{\rho_1}{e_1'}{\beta}
         \\
         \evalBwd{v_2}{U}{\rho_2}{e_2'}{\beta'}
      }
      {
         \evalBwd{\annCons{v_1}{v_2}{\alpha \meet \alpha'}}
                 {\trCons{T}{U}}
                 {\rho_1 \join \rho_2}
                 {\annCons{e_1'}{e_2'}{\alpha \meet \alpha'}}
                 {(\alpha \meet \alpha') \join \beta \join \beta'}
      }
   \end{smathpar}
   }
   &
   (\localref{cons-premise-one}, \localref{cons-premise-two})
   \notag
   \\
   &
   \qedLocal
   (\rho_1 \join \rho_2, \annCons{e_1'}{e_2'}{\alpha \meet \alpha'}, \alpha \meet \alpha')
   \leq
   (\rho, \annCons{e_1}{e_2}{\alpha}, \alpha')
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[lab={\ruleName{$\evalFwdS$-vector}}]
      {
         e \eq \annVec{e_1}{x}{e_2}{\alpha}
         \\
         \evalFwdEq{\rho}{e_2}{\alpha'}{T}
                   {\annInt{j}{\beta}}
         \\
         \evalFwd{\rho \concat \bind{x}{i_{\beta}}}{e_1}{\alpha'}{U_{i}}{v_{i}}
         \quad
         (\forall i \numleq j)
      }
      {
         \evalFwd{\rho}{e}{\alpha'}{\trVec{U_{i}}{x}{i}{T}{j}}
                 {\annVec{v_{i}}{i}{\annInt{j}{\beta}}{\alpha \meet \alpha'}}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \evalBwd{v_i}{U_i}{\rho'_i \concat \bind{x}{i_{\beta^\twoPrime_i}}}{e'_i}{\beta'_i}
   \leq
   \rho \concat \bind{x}{i_{\beta}}, e_1, \alpha'
   \quad
   (\exists \rho'_i, \beta^\twoPrime_i, e'_i, \beta'_i)
   \quad
   (\forall i \numleq j)
   &
   \text{IH}
   \locallabel{vector-premise-two}
   \\
   &
   \evalBwdLeq{\annInt{j}{\beta}}{T}{\rho}{e_2}{\alpha'}
   &
   \text{IH}
   \notag
   \\
   &
   \evalBwd{\annInt{j}{\beta \join \bigjoin\beta^\twoPrime}}{T}{\rho_2}{e_2'}{\alpha^\twoPrime}
   \leq
   \rho, e_2, \alpha'
   \quad
   (\exists \rho_2, e_2', \alpha^\twoPrime)
   &
   \text{$\bigjoin\beta^\twoPrime \leq \beta$; monotonicity}
   \locallabel{vector-premise-one}
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-vector}}
      ]
      {
         \evalBwd{v_{i}}
                 {U_{i}}
                 {\rho'_{i} \concat \bind{x}{i_{\beta^\twoPrime_{i}}}}
                 {e'_{i}}
                 {\beta'_{i}}
         \quad
         (\forall i \numleq j)
         \\
         \evalBwd{\annInt{j}{\beta \join \bigjoin\beta^\twoPrime}}
                 {T}
                 {\rho_2}
                 {e_2'}
                 {\alpha^\twoPrime}
      }
      {
         \evalBwd{\annVec{v_{i}}{i}{\annInt{j}{\beta}}{\alpha \meet \alpha'}}
                 {\trVec{U_{i}}{x}{i}{T}{j}}
                 {\bigjoin\rho' \join \rho_2}
                 {\annVec{\bigjoin e'}{x}{e_2'}{\alpha \meet \alpha'}}
                 {(\alpha \meet \alpha') \join {\bigjoin{\beta'}} \join \alpha^\twoPrime}
      }
   \end{smathpar}
   }
   &
   (\localref{vector-premise-one}, \localref{vector-premise-two})
   \notag
   \\
   &
   \qedLocal
   (\bigjoin\rho' \join \rho_2,
    \annVec{\bigjoin e'}{x}{e_2'}{\alpha \meet \alpha'})
   \leq
   (\rho, \annVec{e_1}{x}{e_2}{\alpha})
   &
   \notag
   \\
   &
   \qedLocal
   (\alpha \meet \alpha') \join {\bigjoin{\beta'}} \join \alpha^\twoPrime
   \leq
   \alpha'
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalFwdS$-vector-lookup}},
         right={$i' \numleq j$}
      ]
      {
         e \eq \exVecAccess{e_1}{e_2}
         \\
         \evalFwdEq{\rho}{e_1}{\alpha}{T}
                   {\annVec{v_{i}}{i}{\annInt{j}{\alpha'}}{\beta}}
      }
      {
         \evalFwd{\rho}{e}{\alpha}{\trVecAccess{T}{j}{U}{i}}{v_{i'}}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \evalBwdLeq{\annVec{v_{i}}{i}{\annInt{j}{\alpha'}}{\beta}}{T}{\rho}{e_1}{\alpha}
   &
   \text{IH}
   \notag
   \\
   &
   \evalBwd{\mapUpdate{\annVec{\hole}{i}{\annInt{j}{\FF}}{\FF}}{i}{v_{i'}}}{T}{\rho_1}{e_1'}{\beta'}
   \leq
   \rho, e_1, \alpha
   \quad
   (\exists \rho_1, e_1', \beta')
   &
   \text{monotonicity}
   \locallabel{vector-lookup-premise-one}
   \\
   &
   \evalFwd{\rho}{e_2}{\alpha}{U}{\annInt{i}{\alpha^\twoPrime}}
   \quad
   (\exists \alpha^\twoPrime)
   &
   \text{codomain of $\evalFwdF{U}$}
   \notag
   \\
   &
   \evalBwdLeq{\annInt{i}{\alpha^\twoPrime}}{U}{\rho}{e_2}{\alpha}
   &
   \text{IH}
   \notag
   \\
   &
   \evalBwd{\annInt{i}{\FF}}{U}{\rho_2}{e_2'}{\beta^\twoPrime}
   \leq
   \rho, e_2, \alpha
   \quad
   (\exists \rho_2, e_2', \beta^\twoPrime)
   &
   \text{monotonicity}
   \locallabel{vector-lookup-premise-two}
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-vector-lookup}}
      ]
      {
         \evalBwd{\mapUpdate{\annVec{\hole}{i}{\annInt{j}{\FF}}{\FF}}{i}{v_{i'}}}
                 {T}{\rho_1}{e_1'}{\beta'}
         \\
         \evalBwd{\annInt{i}{\FF}}{U}{\rho_2}{e_2'}{\beta^\twoPrime}
      }
      {
         \evalBwd{v_{i'}}
                 {\trMatrixAccess{T}{j}{U}{i}}
                 {\rho_1 \join \rho_2}
                 {\exMatrixAccess{e_1'}{e_2'}}
                 {\beta' \join \beta^\twoPrime}
      }
   \end{smathpar}
   }
   &
   (\localref{vector-lookup-premise-one}, \localref{vector-lookup-premise-two})
   \notag
   \\
   &
   \qedLocal
   (\rho_1 \join \rho_2, \exMatrixAccess{e_1'}{e_2'}, \beta \join \beta')
   \leq
   (\rho, \exMatrixAccess{e_1}{e_2}, \alpha)
   &
   \notag
   \\
   \intertext{\crossrule}
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   &
   \caseDerivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[lab={\ruleName{$\evalFwdS$-vector-length}}]
      {
         e \eq \exVecLen{e'}
         \\
         \evalFwdEq{\rho}{e'}{\alpha}{T}
                   {\annot{\exVec{v_{i}}{i}{\annInt{j}{\alpha'}}}{\beta}}
      }
      {
         \evalFwd{\rho}{e}{\alpha}{\trVecLen{T}{j}}
                 {\annInt{j}{\alpha'}}
      }
   \end{smathpar}
   }
   &
   \notag
   \\
   &
   \evalBwdLeq{\annot{\exVec{v_{i}}{i}{\annInt{j}{\alpha'}}}{\beta}}{T}{\rho}{e'}{\alpha}
   &
   \text{IH}
   \notag
   \\
   &
   \evalBwd{\annVec{\hole}{i}{\annInt{j}{\alpha'}}{\FF}}{T}{\rho'}{e^\twoPrime}{\beta}
   \leq
   \rho, e', \alpha
   \quad
   (\exists \rho', e^\twoPrime, \beta)
   &
   \text{monotonicity}
   \locallabel{vector-lookup-premise-one}
   \\
   &
   \qedLocal
   \derivation{\derivationWidth}{
   \begin{smathpar}
      \inferrule*[
         lab={\ruleName{$\evalBwdS$-vector-length}}
      ]
      {
         \evalBwd{\annVec{\hole}{i}{\annInt{j}{\alpha'}}{\FF}}{T}{\rho'}{e^\twoPrime}{\beta}
      }
      {
         \evalBwd{\annInt{j}{\alpha'}}{\trVecLen{T}{j}}{\rho'}{\exVecLen{e^\twoPrime}}{\beta}
      }
   \end{smathpar}
   }
   &
   (\localref{vector-lookup-premise-one}, \localref{vector-lookup-premise-two})
   \notag
   \\
   &
   \qedLocal
   (\rho', \exVecLen{e^\twoPrime}, \beta)
   \leq
   (\rho, \exVecLen{e'}, \alpha)
   &
   \notag
\end{flalign}
\end{proof}
