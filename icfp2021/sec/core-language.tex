...
Each primitive operation $\phi: \tyInt^{i} \to \tyInt$ must for every $\vec{n} \in \dom{\phi}$ provide a Galois connection $(\primFwdBool{\phi}(\vec{n}), \primBwdBool{\phi}(\vec{n}))$ between $\Bool^i$ and $\Bool$, which we lift to a Galois connection $(\primFwd{\phi}, \primBwd{\phi}(\vec{n}))$ between $\Val{\tyInt}^i$ and $\Val{\tyInt}$ by defining
\begin{salign}
   \primFwd{\phi}(\vec{\annInt{n}{\alpha}}) &= \annInt{m}{\beta}
   \text{ where }
   \primFwdBool{\phi}(\vec{n})(\vec{\alpha}) = \beta
   \\
   \primBwd{\phi}(\vec{n})(\annInt{m}{\alpha}) &= \vec{\annInt{n}{\beta}}
   \text{ where }
   \primBwdBool{\phi}(\vec{n})(\alpha) = \vec{\beta}
\end{salign}

\noindent where $\vec{\annInt{n}{\alpha}}$ denotes the zip of $\vec{n}$ and $\vec{\alpha}$ with the constructor for integer values.
...
\begin{lemma}[Determinism of recursive binding]
