...
Each primitive operation $\phi: \tyInt^{i} \to \tyInt$ must for every $\vec{n} \in \dom{\phi}$ provide a Galois connection with the following signature:

\begin{salign}
   \primFwd{\phi}(\vec{n}) &: \Bool^i \to \Bool \\
   \primBwd{\phi}(\vec{n}) &: \Bool \to \Bool^i
\end{salign}

\noindent From these we can derive two forward and backward slicing families

\begin{salign}
   \primFwd{\phi(\vec{n})} &: (\tyInt \times \Bool)^i \to \tyInt \times \Bool \\
   \primBwd{\phi(\vec{n})} &: \tyInt \times \Bool \to (\tyInt \times \Bool)^i
\end{salign}
