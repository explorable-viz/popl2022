\begin{figure}[H]
\flushleft \shadebox{$s \dfwdarrow e$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-application}}
]
{
   s \dfwdarrow e
   \\
   s' \dfwdarrow e'
}
{
   \annot{(s \ s')}{\alpha} \dfwdarrow \annot{(e \ e')}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-empty-list}}
]
{
   \strut
}
{
   \annot{\exNil}{\alpha} \dfwdarrow \annot{\exNil}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-non-empty-list}}
]
{
   s \dfwdarrow e
   \\
   l \dfwdarrow e'
}
{
   \annot{(\sExList{s}{l} \ )}{\alpha} \dfwdarrow \annot{(\exCons{e}{e'})}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-cons}}
]
{
   s \dfwdarrow e
   \\
   s' \dfwdarrow e'
}
{
   \annot{(\exCons{s}{s'})}{\alpha} \dfwdarrow \annot{(\exCons{e}{e'})}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-recursive-function}}
]
{
   \vec{c} \dfwdarrow \sigma
   \\
   s \dfwdarrow e
}
{
   \annot{(\sExLet{f}{\vec{c}}{s})}{\alpha} \dfwdarrow \annot{(\exLetRec{f}{\sigma}{e})}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-match-as}}
]
{
   s \dfwdarrow e
   \\
   \vec{c} \dfwdarrow \sigma
}
{
   \annot{(\exMatch{s}{\vec{c}})}{\alpha}
   \dfwdarrow
   \annot{(\annot{(\lambda \sigma)}{\alpha} \ e)}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-if-then-else}}
]
{
   s \dfwdarrow e
   \\
   s' \dfwdarrow e'
   \\
   s^{\twoPrime} \dfwdarrow e^{\twoPrime}
   \\
   \sigma = \elimBool{e'}{e^{\twoPrime}}
}
{
   \annot{(\exIfThenElse{s}{s'}{s^{\twoPrime}})}{\alpha}
   \dfwdarrow
   \annot{(\annot{(\exLambda{\sigma})}{\alpha} \ e)}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-list-range}},
]
{
   s \dfwdarrow e
   \\
   s' \dfwdarrow e'
}
{
   \annot{\exListSeq{s}{s'}}{\alpha} \dfwdarrow \annot{(\annot{(\kw{range} \ e)}{\alpha}{e'})}{\alpha}
}
%
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-done}}]
{
   s \dfwdarrow e
}
{
   \annot{\exListComp{s}{\annot{\exTrue}{\alpha_1}}}{\alpha_2}
   \dfwdarrow
   \annot{(\exCons{e}{\annot{\exNil}{\alpha_1 \meet \alpha_2}})}{\alpha_1 \meet \alpha_2}
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-qual}},
   right={$q \neq \kw{true}$}
]
{
  \annot{\exListComp{s}{q \comma \annot{\exTrue}{\alpha}}}{\alpha} \dfwdarrow e
}
{
  \annot{\exListComp{s}{q}}{\alpha} \dfwdarrow e
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-guard}}]
{
   \annot{(\exIfThenElse{s'}{\annot{\exListComp{s}{\vec{q}}}{\alpha_2}}{\annot{\exNil}{\alpha_1 \meet \alpha_2}})}{\alpha_1 \meet \alpha_2}
   \dfwdarrow e
}
{
   \annot{\exListComp{s}{\annot{(\qualGuard{s'})}{\alpha_1} \cdot \vec{q}}}{\alpha_2}
   \dfwdarrow e
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-decl}}]
{
   s' \dfwdarrow e'
   \\
   \annot{\exListComp{s}{\vec{q}}}{\alpha_2} \dfwdarrow e
   \\
   \clausenv{p}{s'} \dfwdarrow \sigma
}
{
   \annot{\exListComp{s}{\annot{(\qualDeclaration{p}{s'})}{\alpha_1} \cdot \vec{q}}}{\alpha_2}
   \dfwdarrow
   \annot{(\annot{\exLambda{\sigma}}{\alpha_1 \meet \alpha_2} \ e)}{\alpha_1 \meet \alpha_2}
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-gen}}]
{
   \annot{\clausenv{p}{\exListComp{s}{\vec{q}}}}{\alpha_2} \dfwdarrow \sigma
   \\
   \sigma' = \totalise{p}{e}
   \\
   s' \dfwdarrow e'
}
{
   \annot{\exListComp{s}{\annot{(\qualGenerator{p}{s'})}{\alpha_1} \cdot \vec{q}}}{\alpha_2}
   \dfwdarrow
   \annot{(\annot{(\kw{concatMap} \ \annot{\exLambda{\sigma}}{\alpha_1 \meet \alpha_2})}{\alpha_1 \meet \alpha_2} \ e')}{\alpha_1 \meet \alpha_2}
}
\end{smathpar}
\caption{Desugaring -- forward slicing (additional forms only)}
\end{figure}

\begin{figure}[H]
\flushleft \shadebox{$l \dfwdarrow e$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-end}}
]
{
   \strut
}
{
   \annot{\sExNil}{\alpha} \dfwdarrow \annot{\exNil}{\alpha}
}
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-cons}}
]
{
   s \dfwdarrow e
   \\
   l \dfwdarrow e'
}
{
   \annot{(\sExCons{s}{l})}{\alpha} \dfwdarrow \annot{{(\exCons{e}{e'})}}{\alpha}
}
\end{smathpar}
\caption{Desugaring list rest - forward slicing}
\end{figure}

\begin{figure}[H]
\flushleft\shadebox{$\vec{p}, e \dfwdarrow \sigma$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-pattern-seq}}
]
{
   \vec{p}, e \dfwdarrow \sigma
   \\
   p, \lambda \vec{p} \rightarrow \sigma \dfwdarrow \sigma'
}
{
   p \cdot \vec{p}, e
   \dfwdarrow
   \sigma'
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-var}}
]
{
   \strut
}
{
   x, e
   \dfwdarrow
   \elimVar{x}{e}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-nil}}
]
{
   \strut
}
{
   \pattNil, e
   \dfwdarrow
   \elimList{\branchNil{e}}{\branchCons{\_ \mapsto \_ \mapsto \matchHole}}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-cons}}
]
{
   p', e \dfwdarrow \sigma
   \\
   p, \sigma \dfwdarrow \sigma'
}
{
   \pattCons{p}{p'}, e
   \dfwdarrow
   \elimList{\branchNil{\matchHole}}{\branchCons{\sigma'}}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-non-empty-list}}
]
{
   \pattSCons{p}{o}, e
   \dfwdarrow
   \sigma
}
{
   \pattList{p}{o}, e
   \dfwdarrow
   \sigma
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-pair}}
]
{
   p', e \dfwdarrow \sigma
   \\
   p, \sigma \dfwdarrow \sigma'
}
{
   \pattPair{p}{p'}, e
   \dfwdarrow
   \elimProd{\sigma'}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-list-end}}
]
{
   \strut
}
{
   \pattSNil, e
   \dfwdarrow
   \elimList{\branchNil{e}}{\branchCons{\_ \mapsto \_ \mapsto \matchHole}}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-list-rest}}
]
{
   o, e \dfwdarrow \sigma
   \\
   p, \sigma \dfwdarrow \sigma'
}
{
   \pattSCons{p}{o}, e
   \dfwdarrow
   \sigma'
}
\end{smathpar}
\caption{Desugaring clauses - (Elim) Create eliminator from pattern and continuation}
\end{figure}