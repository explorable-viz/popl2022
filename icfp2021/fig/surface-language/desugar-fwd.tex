\begin{figure}[H]
\flushleft \shadebox{$s \dfwdarrow e$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-non-empty-list}}
]
{
   s \dfwdarrow e
   \\
   l \dfwdarrow e'
}
{
   \annot{(\sExList{s}{l} \ )}{\alpha} \dfwdarrow \annot{(\exCons{e}{e'})}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-if-then-else}}
]
{
   s \dfwdarrow e
   \\
   s' \dfwdarrow e'
   \\
   s^{\twoPrime} \dfwdarrow e^{\twoPrime}
   \\
   \sigma = \elimBool{e'}{e^{\twoPrime}}
}
{
   \annot{(\exIfThenElse{s}{s'}{s^{\twoPrime}})}{\alpha}
   \dfwdarrow
   \annot{(\exLambda{\sigma \ e})}{\alpha}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-list-range}},
]
{
   s \dfwdarrow e
   \\
   s' \dfwdarrow e'
}
{
   \annot{\exListSeq{s}{s'}}{\alpha} \dfwdarrow \annot{(\annot{(\kw{range} \ e)}{\alpha}{e'})}{\alpha}
}
%
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-done}}]
{
   s \dfwdarrow e
}
{
   \annot{\exListComp{s}{\annot{\exTrue}{\alpha_1}}}{\alpha_2}
   \dfwdarrow
   \annot{(\exCons{e}{\exNil})}{\alpha_1 \meet \alpha_2}
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-qual}},
   right={$q \neq \kw{true}$}
]
{
  \annot{\exListComp{s}{q \comma \exTrue}}{\alpha} \dfwdarrow e
}
{
  \annot{\exListComp{s}{q}}{\alpha} \dfwdarrow e
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-guard}}]
{
   \annot{(\exIfThenElse{s'}{\annot{\exListComp{s}{\vec{q}}}{\alpha_2}}{\annot{\exNil}{\alpha_1 \join \alpha_2}})}{\alpha_2}
   \dfwdarrow e
}
{
   \annot{\exListComp{s}{\annot{(\qualGuard{s'})}{\alpha_1} \ \vec{q}}}{\alpha_2}
   \dfwdarrow e
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-decl}}]
{
   s' \dfwdarrow e'
   \\
   \annot{\exListComp{s}{\vec{q}}}{\alpha_2} \dfwdarrow e
   \\
   \sigma = \totalise{p}{e}
}
{
   \annot{\exListComp{s}{\annot{(\qualDeclaration{p}{s'})}{\alpha_1} \comma \vec{q}}}{\alpha_2}
   \dfwdarrow
   \annot{(\exLambda{\sigma} \ e')}{\alpha_1 \join \alpha_2}
}
\and
%
\inferrule*[lab={\ruleName{$\dfwdarrow$-list-comp-gen}}]
{
   \annot{\exListComp{s}{\vec{q}}}{\alpha_2} \dfwdarrow e
   \\
   \sigma = \totalise{p}{e}
   \\
   s' \dfwdarrow e'
}
{
   \annot{\exListComp{s}{\annot{(\qualGenerator{p}{s'})}{\alpha_1} \comma \vec{q}}}{\alpha_2}
   \dfwdarrow
   \annot{(\annot{(\kw{concatMap} \ \exLambda{\sigma})}{\alpha_1 \join \alpha_2} \ e')}{\alpha_1 \join \alpha_2}
}
\end{smathpar}
\caption{Desugaring -- forward slicing (additional forms only)}
\end{figure}

\begin{figure}[H]
\flushleft \shadebox{$l \dfwdarrow e$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-end}}
]
{
   \strut
}
{
   \annot{\sExNil}{\alpha} \dfwdarrow \annot{\exNil}{\alpha}
}
\and
%
\inferrule*[
   lab={\ruleName{$\dfwdarrow$-cons}}
]
{
   s \dfwdarrow e
   \\
   l \dfwdarrow e'
}
{
   \sExConsAnnot{\alpha}{s}{l} \dfwdarrow \annot{{\exCons{e}{e'}}}{\alpha}
}
\end{smathpar}
\caption{Desugaring list rest - forward slicing}
\end{figure}
