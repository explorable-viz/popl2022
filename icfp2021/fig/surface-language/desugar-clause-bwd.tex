\begin{figure}[H]
\flushleft\shadebox{$\sigma, \vec{p} \dbwdarrow e$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-pattern-seq-$\hole$}}
]
{
   \sigma, p \dbwdarrow \hole
   \\
   \hole, \vec{p} \dbwdarrow e
}
{
   \sigma, p \cdot \vec{p}
   \dbwdarrow
   e
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-pattern-seq}}
]
{
   \sigma, p \dbwdarrow \exLambda{\sigma'}
   \\
   \sigma', \vec{p} \dbwdarrow e
}
{
   \sigma, p \cdot \vec{p}
   \dbwdarrow
   e
}
\end{smathpar}

\vspace{5pt}
\flushleft\shadebox{$\sigma, p \dbwdarrow \kappa$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-$\hole$}}
]
{
   p \dbwdhole \sigma
   \\
   \sigma, p \dbwdarrow \kappa
}
{
   \hole, p
   \dbwdarrow
   \kappa
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-var}}
]
{
   \strut
}
{
   \elimVar{x}{\kappa}, x
   \dbwdarrow
   \kappa
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-nil}}
]
{
   \strut
}
{
   \elimListNew{\kappa}{\sigma}, \exNil
   \dbwdarrow
   \kappa
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-cons}}
]
{
   \sigma, p \dbwdarrow \sigma'
   \\
   \sigma', p' \dbwdarrow \kappa'
}
{
   \elimListNew{\kappa}{\sigma}, \pattCons{p}{p'}
   \dbwdarrow
   \kappa'
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-non-empty-list}}
]
{
   \sigma, p \dbwdarrow \sigma'
   \\
   \sigma', o \dbwdarrow \kappa'
}
{
   \elimListNew{\kappa}{\sigma}, \pattList{p}{o}
   \dbwdarrow
   \kappa'
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-pair}}
]
{
   \sigma, p \dbwdarrow \sigma'
   \\
   \sigma', p' \dbwdarrow \kappa
}
{
   \elimProd{\sigma'}, \pattPair{p}{p'}
   \dbwdarrow
   \kappa
}
\end{smathpar}

\vspace{5pt}
\flushleft\shadebox{$\sigma, o \dbwdarrow \kappa$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-list-end}}
]
{
   \strut
}
{
   \elimListNew{\kappa}{\sigma}, \pattListEnd
   \dbwdarrow
   \kappa
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-list-rest}}
]
{
   \sigma, p \dbwdarrow \sigma'
   \\
   \sigma', o \dbwdarrow \kappa'
}
{
   \elimListNew{\kappa}{\sigma}, \pattListNext{p}{o}
   \dbwdarrow
   \kappa'
}
\end{smathpar}

\vspace{5pt}
\flushleft\shadebox{$\sigma, \vec{c} \dbwdarrow \vec{c}$}
\begin{smathpar}
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-clause}}
]
{
   \sigma, \vec{p} \dbwdarrow e
   \\
   e, s \dbwdarrow s'
}
{
   \sigma, \clause{p}{s}
   \dbwdarrow
   \clause{p}{s'}
}
%
\and
%
\inferrule*[
   lab={\ruleName{$\dbwdarrow$-clause-seq}}
]
{
   \sigma, c \dbwdarrow c'
   \\
   \sigma, \vec{c} \dbwdarrow \vec{c}'
}
{
   \sigma, c \cdot \vec{c}
   \dbwdarrow
   c' \cdot \vec{c}'
}
\end{smathpar}

\vspace{5pt}
\begin{minipage}{0.5\textwidth}
\flushleft \shadebox{$p \dbwdhole \sigma$}
\begin{salign}
   \pattVar{x}
   &\dbwdhole
   \elimVar{x}{\hole}
   \\
   \pattNil
   &\dbwdhole
   \elimListNew{\hole}{\hole}
\end{salign}
\end{minipage}%
\begin{minipage}{0.5\textwidth}
\flushleft \shadebox{$o \dbwdhole \sigma$}
\begin{salign}
   \pattListEnd
   &\dbwdhole
   \elimListNew{\hole}{\hole}
   \\
   \pattListNext{p}{o}
   &\dbwdhole
   \elimListNew{\hole}{\hole}
\end{salign}
\end{minipage}
\caption{Desugaring clauses and patterns -- backward slicing}
\end{figure}
