...
As lattices are partially ordered, annotations $\textcal{A}$ also induce a partial order on annotated values $\Sel{v}{A}$, where two values $v$ and $v'$ are related by $v \leq v$ if each annotation in $v$ is below the annotation in the corresponding position in $v'$ (defined in \figref{leq-value-eliminator} and \figref{leq-term}). We can enrich this partial order by introducing hole $\hole$ into the set of annotated values $\Sel{v}{A}$, where holes let us represent any value in a fully unselected state (every annotation position within it is $\bot$).
...
We can consider any value $v$ to be hole-equivalent when every position in its structure is either annotated with $\bot$ or is a hole. For example, the following values are hole-equivalent: $\hole$, $(\annCons{\hole}{\hole}{\bot})$, $(\annCons{\annTrue{\bot}}{\hole}{\bot})$, and $(\annCons{\annTrue{\bot}}{\annFalse{\bot}}{\bot})$.

The equivalence relation induced by the partial order ensures that the equations for zip generalises to holes. As a consequence, it follows that $\hole \join v = v$ and $v \join \hole = v$ for any $v$, and conversely that $\hole \meet v = \hole$ and $v \meet \hole = \hole$.

The notion of holes is important for performance purposes, as we frequently need an efficient way of representing completely unselected values e.g. for environments where only some subset of its bindings are selected.
