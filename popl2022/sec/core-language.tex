...
Each primitive operation $\phi: \tyInt^{i} \to \tyInt$ must for every $\vec{n} \in \dom{\phi}$ provide a Galois connection $(\primFwdBool{\phi}{\vec{n}}, \primBwdBool{\phi}{\vec{n}})$ between $\Bool^i$ and $\Bool$, which we lift to a Galois connection $(\primFwd{\phi}{\vec{n}}, \primBwd{\phi}{\vec{n}})$ between $\Below{\vec{n}}$ and $\Below{\phi(\vec{n})}$ by defining
\begin{definition}
...
\begin{lemma}[Least environment for $\Gamma$]
...
\begin{definition}[Forward and backward functions for environment lookup]
   Suppose $\envLookup{\rho}{z}{v}$. Then define $\envLookupFwdF{\rho,z}: \Below{\rho} \to \Below{v}$ and $\envLookupBwdF{\rho,z}: \Below{v} \to \Below{\rho}$ by the equations in \figref{slicing:eval-aux}.
\end{definition}
...
\begin{definition}[Forward and backward functions for recursive bindings]
   Suppose $\rho, h \closeDefsFwdR \rho'$. Define $\closeDefsFwdF{\rho,h}: \Below{(\rho, h)} \to \Below{\rho'}$ and $\closeDefsBwdF{\rho,h}: \Below{\rho'} \to \Below{(\rho, h)}$ by the equations in \figref{slicing:eval-aux}.
\end{definition}
