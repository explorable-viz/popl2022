...
\begin{figure}[H]
   \flushleft
   \shadebox{$\matchFwdF{w}: \Below{(v,\sigma) \to \Below{(\rho,\kappa, \alpha)}}$}
   and
   \shadebox{$\matchBwdF{w}: \Below{(\rho,\kappa, \alpha)} \to \Below{(v,\sigma)}$}
...
\begin{definition}[Forward and backward functions for pattern-matching]
   Suppose $w: v, \sigma \match \rho, \kappa$. Define $\matchFwdF{w}: \Below{(v,\sigma,\Bool)} \to \Below{(\rho,\kappa)}$ and $\matchBwdF{w}: \Below{(\rho,\kappa)} \to \Below{(v,\sigma,\Bool)}$ as given in \figref{match:fwd-bwd}.
\end{definition}
...
\begin{definition}[Forward and backward functions for evaluation]
   Suppose $T: \rho, e \eval v$. Define $\evalFwdF{T}: \Below{(\rho, e, \Bool)} \to \Below{v}$ and $\evalBwdF{T}: \Below{v} \to \Below{(\rho, e, \Bool)}$ by the equations in \figrefTwo{eval:fwd}{eval:bwd}.
\end{definition}
