...
Figure \ref{fig:core-syntax-type} introduces the types $A, B$ which consist of primitive types $\tyBool$ and $\tyInt$ and function types $\tyFun{A}{B}$, but also lists $\tyList{A}$, vectors $\tyVec{A}$, and records $\tyRec{\vec{\bind{x}{A}}}$ in order to support the kind of fine-grained relationships between input and output that we want to analyse. The notation $\seq{\bind{x}{A}}$ represents a sequence of bindings of variables $x$ to types $A$.
