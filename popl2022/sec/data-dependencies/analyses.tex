...
The rule assumes the application $\exApp{e}{e'}$ already has an ambient availability $\alpha$; at the outermost level this will usually be $\top$. The rule passes $\alpha$ down when recursively forward-evaluating $e$ and $e'$, but computes a new annotation $\beta$ when transferring control to the function. We obtain $\beta$ by forward-matching the argument $v$ with the eliminator $\sigma$ of the closure, and thus it represents the availability of those parts of $v$ demanded by the function. The ambient availability is used to bound the availability of any annotated values constructed in the dynamic context of that function, establishing a dependency between the parts of values consumed by functions, and the parts of values they construct.
