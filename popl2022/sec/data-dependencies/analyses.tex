...
\noindent Galois connections are simple examples of adjoint functors, where $X$ and $Y$ are poset categories, although the right adjoint $f$ is usually called the \emph{upper adjoint} because it approximates an inverse of $g$ from above, and the left adjoint $g$ is called the \emph{lower adjoint} because it approximates an inverse of $f$ from below. Galois connections compose component-wise, so it is useful to think of them as having a domain and a codomain (by convention we use the domain and codomain of the lower adjoint).

We now show that $\evalBwdF{T}$ and $\evalFwdF{T}$ form a Galois connection for any $\Ann{A}$ (\thmref{core-language:eval:gc}) by first establishing that the relevant auxiliary functions also form Galois connections.
...
\label{lem:core-language:env-get-put}Suppose $\envLookup{\raw{\rho}}{x}{\raw{v}}$. Then $(\envLookupBwdF{-}{\raw{\rho}}{x}{},\envLookupFwdF{}{\raw{\rho}}{x}{-}): \Sel{\raw{v}}{A} \to \Sel{\raw{\rho}}{A}$ is a Galois connection. \todo{better notation for environment lookup functions}
\end{lemma}
...
We assume (rather than prove) that the forward and backwards dependency functions $\primGCBool{\phi}{\vec{n}} = (\primFwdBool{\phi}{\vec{n}}, \primBwdBool{\phi}{\vec{n}})$ provided for a primitive operation $\phi: \tyInt^i \to \tyInt$ form a Galois connection of type $\Ann{A} \to \Ann{A}^i$. Under this assumption the following holds.
