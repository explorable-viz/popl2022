...
The forward and backward analysis functions can be used to answer the kind of question we motivated in \secref{introduction:data-linking}, such as ``What data is needed to compute this bar in a bar chart?'' (We used our implementation to generate \figref{introduction:data-linking} that illustrated this use case.) In the introduction we also motivated the idea that certain round-tripping properties might be relevant. For example, given an input selection computed by $\evalBwdF{T}$, we might ask whether it contains the data required to reconstruct --- whether it is \emph{sufficient for} --- the selected part of the output. Dually, given an output selection computed by $\evalFwdF{T}$, we might ask whether it contains too much output to generated by a given data selection. These considerations points towards \emph{Galois connections} as a way of understanding the relationship between $\evalBwdF{T}$ and $\evalFwdF{T}$. Galois connections generalise isomorphisms in a setting where the function domains are partially ordered: they are pair of functions that are not quite mutual inverses, but are the nearest to an inverse each can get to the other. \todo{complete}

\todo{Better notation for environment lookup functions.}
...
\label{lem:core-language:env-get-put}Suppose $\envLookup{\raw{\rho}}{x}{\raw{v}}$. Then $(\envLookupFwdF{-}{\raw{\rho}}{x}{}) \adjoint (\envLookupBwdF{}{\raw{\rho}}{x}{-})$.
\end{lemma}
