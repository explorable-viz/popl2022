...
The analyses above can be used to answer the kind of question we motivated in \secref{introduction:data-linking}, such as ``What data is needed to compute this bar in a bar chart?'' (We used our implementation to generate \figref{introduction:data-linking}.) In order to trust the answer, we need to know that the analyses satisfy certain correctness properties. Intuitively, we expect the backward analysis $\evalBwdF{T}$ to be ``sound'': to pick out all (and hopefully only) the input data required to reconstruct the output selection. Similarly, we expect the forward analysis $\evalFwdF{T}$ to be ``complete'': to pick out only (and hopefully all) output data that can be computed from the selected input. We can understand these requirements as \emph{round-tripping} properties: we expect $\evalFwdF{T}(\evalBwdF{T}(v))$ to produce a value selection $v' \geq v$, and $\evalBwdF{T}(\evalFwdF{T}(\rho,e))$ to produce an input selection $(\rho',e') \leq (\rho,e)$.

Pairs of (monotonic) functions that satisfy these round-tripping properties are called \emph{Galois connections}. Galois connections generalise isomorphisms in a setting where the function domains are partially ordered: they are not quite mutual inverses, but are the nearest to an inverse each can get to the other.

\begin{definition}[Galois connection]
   Suppose $X$ and $Y$ are posets with partial orders $\numleq_X$ and $\numleq_Y$. Then monotonic functions $f: X \to Y$ and $g: Y \to X$ form a \emph{Galois connection} iff $f(g(y)) \numgeq_Y y$ and $g(f(x)) \numleq_X x$.
\end{definition}

\noindent We write $f \adjoint g$ if $f$ and $g$ form a Galois connection. This is justified by the fact that Galois connections are also a simple example of adjoint functors, where the categories in question are poset categories and the $f$ and $g$ are functors. In this context the right adjoint $f$ is called the \emph{upper adjoint} and the left adjoint is called the \emph{lower adjoint}.
...
\label{lem:core-language:env-get-put}Suppose $\envLookup{\raw{\rho}}{x}{\raw{v}}$. Then $(\envLookupFwdF{-}{\raw{\rho}}{x}{}) \adjoint (\envLookupBwdF{}{\raw{\rho}}{x}{-})$. \todo{Better notation for environment lookup functions.}
\end{lemma}
...
\label{thm:core-language:closeDefs:gc}
   Suppose $\raw{\rho}, \raw{h} \closeDefsR \raw{\rho}'$.  Then ${\closeDefsFwdF{\raw{\rho},\raw{h}}} \adjoint {\closeDefsBwdF{\raw{\rho},\raw{h}}}$.
\end{theorem}
...
We assume (rather than prove) that the forward and backwards dependency functions provided for a primitive operation $\phi: \tyInt^i \to \tyInt$ form a Galois connection between $\Ann{A}^i$ and $\Ann{A}$, i.e.~that $\primFwdBool{\phi}{\vec{n}} \adjoint \primBwdBool{\phi}{\vec{n}}$. Under this assumption the following holds.
...
One concern might be that establishing that $\evalFwdF{T}$ and $\evalBwdF{T}$ form an adjoint pair is rather weak correctess property, since it provides no independent reason for thinking that the definition is good. And it is certainly true that one could define a trivial $\evalFwdF{T}$ whose lower adjoint would be equally trivial, much as one can usually define trivial analyses where the correctness properties are given by soundness or completeness. However, for a non-trivial definition, the Galois connection property is quite strong, and we are unaware of any related work (for example on Galois slicing) which improves on this state of affairs.
