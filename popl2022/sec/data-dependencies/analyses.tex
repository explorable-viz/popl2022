...
\figref{data-dependencies:fwd} also defines the family of functions $\evalFwdF{T} :: \Prod{\Sel{\rho, e}{A}}{\Ann{A}} \to \Sel{v}{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$, pronounced ``forward evaluation''. This too should be understood as a total function, defined by structural recursion on $T$, which replays the evaluation, using the input selection $\rho, e \in \Sel{\rho,e}{A}$ to determine the output selection $v \in \Sel{v}{A}$. The additional annotation input $\alpha$ represents the ``availability'' (in the sense of \secref{data-dependencies:analyses:fwd:pattern-match}) of the argument consumed by the ambient function context, which bounds the availability of any resources constructed in that context. The availability of any annotated values constructed during evaluation is the meet of the availability of the annotated inputs they depend on, and this argument availability. At the outermost level there is no ambient function context, and so the the forward evaluation relation is typically invoked initially with an argument availability of $\top$.

Analogously to $\matchFwdF{w}$ and $\matchR$, the rules for $\evalFwdF{T}$ mirror those of the evaluation relation $\evalR$. The variable rule simply returns the annotated value bound to $x$ in $\rho$.  The rule for lambdas $\exLambda{\sigma}$ preserves any annotations in the environment $\rho$ and eliminator $\sigma$ into the resulting closure; the closure has no annotations of its own, so the argument availability $\alpha$ is disregarded. Record projection is more interesting, preserving any annotations internal to the projected field $v_i$ but disregarding not only the ambient $\alpha$ but also the annotation $\beta$ on the record itself. The guiding principle is that ``containers'' are, in general, independent of the values they contain; here, $v_i$ has its own internal annotations which are preserved, but there is no data dependency of the field on the record itself. Record construction also reflects this principle, by forward evaluating the subcomputation for each field and transferring the resulting annotated value into the record unchanged. But it also annotates the record value with the meet of $\alpha$ and $\alpha'$, specifying that the container itself requires both the constructor expression and any resources needed by the ambient function. This reflects a second principle, viz.~that making a value in a function context requires at least the resources that the function required. The rules for nil, cons and integers are similar, although lists have an annotation at every node, unlike records, which have a single annotation.
