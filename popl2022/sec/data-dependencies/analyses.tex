...
\figref{data-dependencies:bwd} also defines the family of functions $\evalBwdF{T}: \Sel{\raw{v}}{A} \to \Sel{\raw{\rho}, \raw{e}}{A} \times \Ann{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$, pronounced ``backward evaluation''. Backward evaluation rewinds $T$, using the output selection $v \in \Sel{\raw{v}}{A}$ to determine an input selection $\rho, e \in \Sel{\raw{\rho}, \raw{e}}{A}$. The rules resemble those of the evaluation relation $\evalR$ with the inputs and outputs flipped, but with an additional output $\alpha$ called the \emph{ambient demand} representing the join of the demand on any values constructed in the current function context.

Again we start by explaining the application rule, which is the only place where ambient demands are used and function contexts terminated. The rule essentially runs the forward evaluation rule in reverse, using the trace $T'$ to backward-evaluate the function body. The obtained $\beta$ is the join of the demand on any resources constructed directly by that function invocation, and is transferred to the matched part of the function argument by backward-matching the argument, establishing a link between resources consumed and resources produced. The evaluations of take place in the containing function context, unrelated to $\beta$, and so the ambient demand for the application is the join of any resources used by $e_1$ and $e_2$. The family of auxiliary functions $\sub{\closeDefsBwdR}{\rho, h}: \Sel{\raw{\rho'}}{A} \to \Sel{\raw{\rho}, \raw{h}}{A}$ for any $\raw{\rho}, \raw{h} \closeDefsR \raw{\rho'}$, defined at the bottom of \figref{data-dependencies:bwd}, is used to turn the closure environment $\rho_2$, capturing the demand on any recursive uses of the function and any functions with which it was mutually defined, into information that can be merged back into the demand on the closure itself.
