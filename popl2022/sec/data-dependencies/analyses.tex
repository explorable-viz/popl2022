...
\figref{data-dependencies:bwd} defines the family of backward-analysis functions $\matchBwdF{w} :: \Prod{\Sel{\raw{\rho}, \raw{\kappa}}{A}}{\Ann{A}} \to \Sel{\raw{v}, \raw{\sigma}}{A}$ for any $w :: \raw{v}, \raw{\sigma} \matchR \raw{\rho}, \raw{\kappa}$, pronounced ``backward match''. Backward-match rewinds the match witnessed by $w$, propagating output selections to input selections. First, the selection on the continuation $\kappa \in \Sel{\raw{\kappa}}{A}$ becomes a selection $\sigma \in \Sel{\raw{\sigma}}{A}$ on original eliminator $\raw{\sigma}$, which contains $\raw{\kappa}$. Second, the value selections in $\rho \in \Sel{\raw{\rho}}{A}$ are transferred back to a selection $v \in \Sel{\raw{v}}{A}$ of the original matched value. Finally, there is an additional argument $\alpha$ called the \emph{ambient demand}, representing the extent to which the matched portion of $\raw{v}$ was needed downstream, which is also merged into $v$.
