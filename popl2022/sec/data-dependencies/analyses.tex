...
Each primitive operation $\phi: \tyInt^{i} \to \tyInt$ must for every $\vec{n}$ of length $i$ provide a Galois connection $(\primFwdBool{\phi}{\vec{n}}, \primBwdBool{\phi}{\vec{n}})$ between $\Ann{A}^i$ and $\Ann{A}$, which we lift to a Galois connection $(\primFwd{\phi}{\vec{n}}, \primBwd{\phi}{\vec{n}})$ between $\Sel{\vec{n}}{A}$ and $\Sel{\phi(\vec{n})}{A}$ by defining
\begin{definition}
