...
Establishing that $\evalBwdF{T}$ and $\evalFwdF{T}$ form an adjoint pair might seem rather weak as a correctess property. It offers no independent grounds for thinking that the definitions are useful, but merely ensures that they are related in a sensible way. This is a familiar problem from approximate analyses like type systems and model checking, where properties like soundness or completeness do not by themselves guarantee utility. And it is certainly true that one could define versions of $\evalBwdF{T}$ and $\evalFwdF{T}$ that are too coarse grained to be useful, yet still satisfy \thmref{core-language:eval:gc}. However, for non-trivial definitions, being a Galois connection imposes quite a strong constraint: every tightening or tweak to the forward analysis implies the corresponding adjustment to the backward analysis, and vice-versa. In \secref{conclusion} we consider how other ideas from provenance and program slicing might provide additional correctness criteria.
