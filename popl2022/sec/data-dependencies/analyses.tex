...
\figref{data-dependencies:fwd} also defines the family of functions $\evalFwdF{T} :: \Prod{\Sel{\rho, e}{A}}{\Ann{A}} \to \Sel{v}{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$, pronounced ``eval-forward''. This too should be understood as a total function, defined by structural recursion on $T$, which replays the evaluation witnessed by $T$, using the input selection $\rho, e \in \Sel{\rho,e}{A}$ to determine the output selection $v \in \Sel{v}{A}$. The additional annotation argument represents the ``availability'' of the argument associated with the ambient function context (in the sense described in \secref{data-dependencies:analyses:fwd:pattern-match}), which constraints the availability of any resources constructed in that context. The availability of any annotated values constructed during evaluation is the meet of the availability of the annotated inputs they depend on, the ambient function argument. At the outermost level there is no ambient function context, and so the argument availability is initially $\top$.

Analogously to $\matchFwdF{w}$ and $\matchR$, the rules for $\evalFwdF{T}$ mirror those of the evaluation relation $\evalR$. The variable rule simply returns the annotated value bound to $x$ in $\rho$. Record projection is similar, preserving any annotations on the field value $v_i$. The rule for lambda expressions $\exLambda{\sigma}$ simply preserves any annotations in the environment $\rho$ and eliminator $\sigma$ into the resulting closure, which has no annotations at the top level. The rules that construct annotated values are more interesting. The rule for annotated integer constants $\annInt{n}{\alpha}$ annotates the resulting integer with the meet of $\alpha$ and the ambient argument availability $\alpha'$, expressing that the value $n$ requires both the expression that constructs it \emph{and} any resources needed by the function which constructed it. The nil rule is similar.
