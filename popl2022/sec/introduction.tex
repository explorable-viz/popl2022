...
Techniques for analysing the dependency structure of programs abound. While these approaches have been very fruitful, with applications ranging from information-flow security \cite{sabelfeld03} and optimisation \cite{kildall73} to debugging and program comprehension \cite{weiser81,delucia96}, techniques suitable for analysing richly structured outputs such as data visualisations and matrices are in short supply. Where-provenance \cite{buneman01} and related data provenance techniques can be fine-grained but are restricted to relational query languages. Taint tracking to dataflow analysis and program slicing \Todo{citations}. For programs which transform richly structured inputs into richly structured outputs, such as visualisation code which turns tables of data into charts and other graphics, or array computations which operate on large multidimensional structures, we need analysis tools that let us focus on a particular part of the output, and isolate the input dependencies pertinent only to that part of the output.

Program slicing techniques which allow the user to focus on part of the output by ``erasing'' those parts which deemed to be irrelevant and propagating these holes backwards to similarly discard parts of the program and input. However, these restrict the notion of ``substructure'' to trees where some subtrees have been erased, rather than being able to focus on a subtree. While
