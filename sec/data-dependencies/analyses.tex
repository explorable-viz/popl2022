...
\vspace{2mm}
\noindent The following property means that an implementation is free to replace any term by a hole-equivalent one of the same shape, with the result of $\matchFwdF{w}$ being unique up to $\eq$. Our implementation expands terms only until a non-hole rule applies; there will be exactly one such rule, corresponding to the execution path originally taken, and although there will in general be multiple expansions that cause that rule to fire, the result will be hole-equivalent.
