...
\figref{data-dependencies:fwd} also defines a family of \emph{forward-evaluation} functions $\evalFwdF{T}$ of type $(\Sel{\raw{\rho}, \raw{e}}{A}) \times \Ann{A} \to \Sel{\raw{v}}{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$. (Like forward match, forward evaluation is presented in a relational style, but should be read as a total function defined by structural recursion on $T$.) Forward evaluation replays $T$, turning a selection $(\rho, e) \in \Sel{\raw{\rho}, \raw{e}}{A}$ identifying the available inputs into a selection $v \in \Sel{\raw{v}}{A}$ identifying the outputs that can be produced using only those inputs. Function calls have an especially important role. Using the forward-match function $\matchFwdF{w}$, a function call determines the availability of any consumed inputs, and takes their meet $\alpha$; any outputs constructed in the dynamic context of that function call are then assigned an availability of $\alpha$, establishing a dependency between resources consumed and resources produced. Since the dynamic context extends over multiple evaluation steps, we use an additional argument to $\evalFwdF{T}$ to track $\alpha$. This additional argument, representing the input availability for the current function call, we call the \emph{ambient availability}, and at the top level, before there are any active function calls, it has the value $\top$. Otherwise the structure of forward evaluation mirrors that of the evaluation relation $\evalR$.
