...
We assume (rather than prove) that the backward and forward dependency functions $\primGCBool{\phi}{\vec{n}} = (\primBwdBool{\phi}{\vec{n}},\primFwdBool{\phi}{\vec{n}})$ provided for every primitive operation $\phi: \tyInt^i \to \tyInt$ and every $\vec{n}$ of length $i$ form a Galois connection of type $\Ann{A} \to \Ann{A}^i$. Under this assumption the following holds.
