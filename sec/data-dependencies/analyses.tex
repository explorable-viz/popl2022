...
Operationally, these hole rules can be interpreted as ``expanding'' the holes in $v$ or $\sigma$, in a shape-preserving way, until another rule of the definition applies. Recall the pattern-matching example from \secref{core-language:pattern-match}. This pattern-match has the witness $\matchCons{\kw{x}}{\kw{xs}}$, recording that the list \kw{\exCons{5}{\exCons{6}{\exNil}}} was matched to the depth of a single cons constructor. Suppose we wish to forward-match over this computation using $\hole$ to represent the selection on the list. The information in the match witness allows us to expand $\hole$ to $\annCons{\hole}{\hole}{\bot}$ and then use the $\matchFwdS$-cons rule to derive the following forward-match:
...
\vspace{2mm}
\noindent \lemref{match-fwd:monotonic} below implies that an implementation is free to replace any term by a hole-equivalent one of the same shape, with the result of $\matchFwdF{w}$ being unique up to $\eq$. This justifies the strategy of expanding holes just enough for a non-hole rule to apply; there will be exactly one such rule which applies, corresponding to the execution path originally taken, and although there may be multiple possible expansions, they will produce hole-equivalent results. This also explain why it is reasonable to think of $\matchFwdF{w}$ as a function, rather than a relation.
...
\figref{data-dependencies:fwd} also defines a family of \emph{forward-evaluation} functions $\evalFwdF{T}$ of type $(\Sel{\raw{\rho}, \raw{e}}{A}) \times \Ann{A} \to \Sel{\raw{v}}{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$. (Like forward match, forward evaluation is presented in a relational style, but should be read as a total function defined by structural recursion on $T$.) Forward evaluation replays $T$, turning a selection $(\rho, e) \in \Sel{\raw{\rho}, \raw{e}}{A}$ identifying the available inputs into a selection $v \in \Sel{\raw{v}}{A}$ identifying the outputs that can be produced using only those inputs.

Unsurprisingly, function calls have an especially important role in establishing input-output dependencies. Forward-evaluating a function call must determine the availability of any consumed inputs, and take their meet $\alpha$, using the forward-match function $\matchFwdF{w}$ defined above; a dependency between the matched part of the argument and any value parts produced by the function are then established by ensuring that any outputs constructed in the dynamic context of the function call have an availability upper-bounded by $\alpha$. Since the dynamic context of a function call extends over multiple evaluation steps, we use an additional argument to $\evalFwdF{T}$ to track the availability $\alpha$ of the matched part of the argument; we call this the \emph{ambient availability}, and at the top level, before there are any active function calls, it has the value $\top$. Moreover, when a lambda is evaluated to a closure, it must capture the ambient availability for use when its dynamic context is resumed. With this preamble out of the way, we now explain individual rules in more detail.
