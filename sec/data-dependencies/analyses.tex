...
\figref{data-dependencies:fwd} also defines a family of \emph{forward-evaluation} functions $\evalFwdF{T}$ of type $(\Sel{\raw{\rho}, \raw{e}}{A}) \times \Ann{A} \to \Sel{\raw{v}}{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$. (Like forward match, forward evaluation is presented in a relational style, but should be read as a total function defined by structural recursion on $T$.) Forward evaluation replays $T$, turning a selection $(\rho, e) \in \Sel{\raw{\rho}, \raw{e}}{A}$ identifying the available inputs into a selection $v \in \Sel{\raw{v}}{A}$ identifying the outputs that can be produced using only those inputs. Input-output dependencies are established through functions calls (which demand non-primitive inputs and construct values) and primitive operations (which demand primitive inputs and construct values).
