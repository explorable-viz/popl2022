...
\vspace{2mm}
\noindent \lemref{match-fwd:monotonic} below implies that an implementation is free to replace any term by a hole-equivalent one of the same shape, with the result of $\matchFwdF{w}$ being unique up to $\eq$. This justifies the strategy of expanding holes just enough for a non-hole rule to apply; there will be exactly one such rule, corresponding to the execution path originally taken, and although there may be multiple possible expansions, they will produce hole-equivalent results. This also explains why it is reasonable to think of $\matchFwdF{w}$ as a function, rather than a relation.
...
The so-called ``argument availability'' $\alpha$ returned by $\matchFwdF{w}$ has a important role in establishing input-output dependencies. When a function constructs a value, the availability of the constructed value is upper-bounded by the argument availability $\alpha$ that arose from pattern-matching the function's argument. This establishes a dependency between data consumed and data constructed. Since the dynamic context of a function call extends over multiple evaluation steps, the forward evaluation function $\evalFwdF{T}$, to which we now turn, takes an additional input which tracks the dynamically innermost argument availability $\alpha$; at the top level, before there are any active function calls, this has the value $\top$. Moreover, closures must capture the active argument availability $\alpha$ when they are constructed so that it can be restored when they are executed later.

\subsubsection{Forward evaluation}
\label{sec:data-dependencies:forward-eval}

\figref{data-dependencies:fwd} also defines a family of \emph{forward-evaluation} functions $\evalFwdF{T}$ of type $(\Sel{\raw{\rho}, \raw{e}}{A}) \times \Ann{A} \to \Sel{\raw{v}}{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$. (Like forward match, forward evaluation is presented in a relational style, but should be read as a total function defined by structural recursion on $T$.) Forward evaluation over $T$ takes as input a selection $(\rho, e) \in \Sel{\raw{\rho}, \raw{e}}{A}$ identifying the available parts of the environment and program, and an $\alpha \in \Ann{A}$ representing the active argument availability. The result is a selection $v \in \Sel{\raw{v}}{A}$ identifying the outputs that can be produced using only those resources.

\Paragraph{Function application} The application rule uses the current argument availability $\alpha$ to recursively forward-evaluate $e$ and $e'$, but must compute a new argument availability for the function body to reflect the change in function context. First, it must unpack the $\beta$ captured into the closure, which is the argument availability active when the closure was constructed, and combine it (using $\meet$) with the $\beta'$ obtained by forward-matching the argument $v$ with the eliminator $\sigma$ of the closure, representing the availability of those parts of $v$ demanded by the new function. These are combined (using $\meet$), and so represent the conjoined availability of \emph{all} the arguments that had to be pattern-matched in order to execute the function body. This becomes the argument availability for the function body. The auxiliary function $\closeDefsFwdF{\rho,h}: (\Sel{\raw{\rho},\raw{h}}{A}) \times \Ann{A} \to \Sel{\raw{\rho}'}{A}$ for any $\smash{\raw{\rho}, \raw{h} \closeDefsR \raw{\rho'}}$ is given at the bottom of \figref{data-dependencies:fwd} and follows $\closeDefsR$, but captures the active argument availability into each closure.
...
\Paragraph{Other rules} All other rules pass the ambient availability into any subcomputations unchanged. Variable lookup disregards the ambient $\alpha$, simply preserving the selection on the value extracted from the environment. The lambda rule captures the ambient availability in the closure, along with the environment; the letrec rule passes it on to $\closeDefsFwdR$ so that it is captured by recursive closures as well. Record projection is more interesting, disregarding not only the ambient $\alpha$ but also the availability $\beta$ of the record itself. Containers are considered to be independent of the values they contain: here, $v_i$ has its own internal availability which is preserved by projection, but there is no implied dependency of the field on the record from which it was projected. Record construction also reflects this principle, preserving the field selections unchanged into the resulting record selection. But this rule also sets the availability of the record value to be $\alpha \meet \alpha'$, reflecting the dependency of the constructed container on both the constructing expression and the ambient availability. The rules for nil, cons and integers are similar.
