...
\Paragraph{Function application} This rule is where the function context changes and so a new argument availability must be determined. First, the rule unpacks the $\beta$ stored in the closure, which represents the argument availability which was active when the closure was constructed. Then the argument availability $\beta'$, representing the availability of the part of the argument $v$ matched by the function, is determined by forward-matching $v$ with the eliminator $\sigma$ of the closure. These are combined using $\meet$ to represent the conjoined availability of \emph{all} the arguments that were pattern-matched in order to execute the function body. This result $\beta \meet \beta'$ becomes the new argument availability used to forward-evaluate the function body. The auxiliary function $\closeDefsFwdF{\rho,h}: (\Sel{\raw{\rho},\raw{h}}{A}) \times \Ann{A} \to \Sel{\raw{\rho}'}{A}$ for any $\smash{\raw{\rho}, \raw{h} \closeDefsR \raw{\rho'}}$ is given at the bottom of \figref{data-dependencies:fwd} and follows $\closeDefsR$, but captures the active argument availability into each closure.
