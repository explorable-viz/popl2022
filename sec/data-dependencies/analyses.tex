...
Operationally, these hole rules can be interpreted as ``expanding'' the holes in $v$ or $\sigma$, in a shape-preserving way, until another rule of the definition applies. Recall the pattern-matching example from \secref{core-language:pattern-match}, which has the match witness $\matchCons{\kw{x}}{\kw{xs}}$ recording that the original match had a cons at the root. Suppose we wish to forward-match using $\hole$ to represent the selection on the original list \kw{\exCons{5}{\exCons{6}{\exNil}}}. Using the match witness, we can expand $\hole$ to $\annCons{\hole}{\hole}{\bot}$ and use the $\matchFwdS$-cons rule to derive a forward-match:
...
\vspace{2mm}
\noindent The following property means that an implementation is free to replace any term by a hole-equivalent one of the same shape, with the result of $\matchFwdF{w}$ being unique up to $\eq$. This justifies the strategy of expanding terms only until a non-hole rule applies; there will be exactly one such rule, corresponding to the execution path originally taken, and although there may be multiple expansions that cause that rule to fire, they produce hole-equivalent results.
