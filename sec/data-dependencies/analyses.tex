...
Operationally, these hole rules can be interpreted as ``expanding'' the holes in $v$ or $\sigma$, in a shape-preserving way, until another rule of the definition applies. Recall the pattern-matching example from \secref{core-language:pattern-match}, which has the match witness $\matchCons{\kw{x}}{\kw{xs}}$ recording that the matched value had a cons at the root. Suppose we wish to forward-match over this computation using $\hole$ to represent the selection on the original list \kw{\exCons{5}{\exCons{6}{\exNil}}}. Using the information in the match witness, we can expand $\hole$ to $\annCons{\hole}{\hole}{\bot}$ and then use the $\matchFwdS$-cons rule to derive the following forward-match:
