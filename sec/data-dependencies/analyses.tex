...
One might hope to dispense with the need for $w$ by simply defining $\matchFwdS$ by case analysis on $v$ and $\sigma$. However, it is then unclear how to proceed in the event that either $v$ or $\sigma$ is a hole. In particular, it is not clear how to obtain the $\raw{\rho}$ associated with the original pattern-match in order to produce an environment selection $\rho' \in \Sel{\raw{\rho}}{A}$. If $\matchFwdS$ is defined with respect to a known $w$, this can be achieved via additional rules \ruleName{$\matchFwdS$-hole-$v$} and \ruleName{$\matchFwdS$-hole-$\sigma$} that define the behaviour at hole to be the same as the behaviour at any $\eq$-equivalent value in $\Sel{\raw{v}}{A}$ or $\Sel{\raw{\sigma}}{A}$.

Operationally, these hole rules can be interpreted as ``expanding'' the holes in $v$ or $\sigma$, in a shape-preserving way, until another rule of the definition applies. Recall the pattern-matching example from \secref{core-language:pattern-match}, and suppose we wished to forward-match using $\hole$ to represent the selection on the original list \kw{\exCons{5}{\exCons{6}{\exNil}}}. The proof term $\matchCons{\kw{x}}{\kw{xs}}$ witnesses that the original match had a cons at the root; this allows us to expand $\hole$ to $\annCons{\hole}{\hole}{\bot}$ and use the $\matchFwdS$-cons rule to derive a forward-match:
...
\noindent There will be exactly one non-hole rule that applies as a result of hole expansion, corresponding to the execution path originally taken, and although there may be multiple such expansions, the following property implies that the result of $\matchFwdF{w}$ will be unique up to $\eq$.
