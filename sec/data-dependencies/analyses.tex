...
\figref{data-dependencies:fwd} also defines a family of \emph{forward-evaluation} functions $\evalFwdF{T}$ of type $(\Sel{\raw{\rho}, \raw{e}}{A}) \times \Ann{A} \to \Sel{\raw{v}}{A}$ for any $T :: \raw{\rho}, \raw{e} \evalR \raw{v}$. (Like forward match, forward evaluation is presented in a relational style, but should be read as a total function defined by structural recursion on $T$.) Forward evaluation replays $T$, turning a selection $(\rho, e) \in \Sel{\raw{\rho}, \raw{e}}{A}$ identifying the available inputs into a selection $v \in \Sel{\raw{v}}{A}$ identifying the outputs that can be produced using only those inputs. Functions calls and primitive operations determine the availability of any consumed inputs; the meet $\alpha$ of these then becomes the availability of any outputs constructed in the dynamic context of that operation, establishing a dependency between resources consumed and resources produced. For primitives, this is straightforward as the entire operation executes as a black box. For function calls, the invocation extends for multiple evaluation steps, and so we must use an additional argument to $\evalFwdF{T}$ to track $\alpha$; we call this input availability for the current function call the \emph{ambient availability}. At the top level, before there are any active function calls, this is set to $\top$. Otherwise the structure of forward evaluation mirrors that of the evaluation relation $\evalR$.
