...
Operationally, these hole rules can be interpreted as ``expanding'' the holes in $v$ or $\sigma$, in a shape-preserving way, until another rule of the definition applies. Recall the pattern-matching example from \secref{core-language:pattern-match}, which has the match witness $\matchCons{\kw{x}}{\kw{xs}}$ recording that the matched list \kw{\exCons{5}{\exCons{6}{\exNil}}} was matched to a depth of a single cons constructor. Suppose we wish to forward-match over this computation using $\hole$ to represent the selection on the original list. Using the information in the match witness, we can expand $\hole$ to $\annCons{\hole}{\hole}{\bot}$ and then use the $\matchFwdS$-cons rule to derive the following forward-match:
...
\vspace{2mm}
\noindent The following property implies that an implementation is free to replace any term by a hole-equivalent one of the same shape, with the result of $\matchFwdF{w}$ being unique up to $\eq$. This justifies the strategy of expanding holes just enough for a non-hole rule to apply, and thinking of $\matchFwdF{w}$ as a function, rather than a relation. There will be exactly one such non-hole rule which applies, corresponding to the execution path originally taken, and although there may be multiple expansions that cause it to be applicable, they produce hole-equivalent results.
