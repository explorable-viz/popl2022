\begin{figure}
\flushleft \shadebox{$\Gamma \vdash e: A$}
\begin{smathpar}
\inferrule*
{
   \strut
}
{
   \Gamma \vdash \bot: \bot
}
%
\and
%
\inferrule*[right={$x : A \in \Gamma$}]
{
   \strut
}
{
   \Gamma \vdash \exVar{x}: A
}
%
\and
%
\inferrule*
{
   \strut
}
{
   \Gamma \vdash n: \tyInt
}
%
\and
%
\inferrule*
{
   \strut
}
{
   \Gamma \vdash \exTrue: \tyBool
}
%
\and
%
\inferrule*
{
   \strut
}
{
   \Gamma \vdash \exFalse: \tyBool
}
%
\and
%
\inferrule*
{
   \Gamma \vdash u: A
   \\
   \Gamma \vdash v: B
}
{
   \Gamma \vdash \exPair{u}{v}: \tyProd{A}{B}
}
%
\and
%
\inferrule*
{
   \Gamma \vdash u: A
}
{
   \Gamma \vdash \exPairProjL{v}: \tyProdProjL{A}
}
%
\and
%
\inferrule*
{
   \Gamma \vdash u: A
}
{
   \Gamma \vdash \exPairProjR{v}: \tyProdProjR{A}
}
%
\and
%
\inferrule*
{
   \Gamma \vdash e: \tyInt
   \\
   \Gamma \vdash e': \tyInt
}
{
   \Gamma \vdash \exAdd{e}{e'}: \tyInt
}
%
\and
%
\inferrule*
{
   \cxtExtend{\Gamma}{f}{\tyFun{A}{B}} \vdash \sigma: \tyFun{A}{B}
}
{
   \Gamma \vdash \exRec{f}{\sigma}: \tyFun{A}{B}
}
%
\and
%
\inferrule*
{
   \exprType{e}{\Gamma}{\tyFun{A}{B}}
   \\
   \exprType{e'}{\Gamma}{A}
}
{
   \exprType{\exApp{e}{e'}}{\Gamma}{B}
}
%
\and
%
\inferrule*
{
   \Gamma \vdash e: A
   \\
   \cxtExtend{\Gamma}{x}{A} \vdash e': B
}
{
   \Gamma \vdash \exLet{x}{e}{e'}: B
}
%
\and
%
\inferrule*
{
   \cxtExtend{\Gamma}{x}{\bot} \vdash e: A
}
{
   \Gamma \vdash \exLetProj{x}{e}: A
}
%
\and
%
\inferrule*
{
   \strut
}
{
   \Gamma \vdash \exNil: \tyList{\tyInt}
}
%
\and
%
\inferrule*
{
   \Gamma \vdash u: \tyInt
   \\
   \Gamma \vdash v: \tyList{\tyInt}
}
{
   \vdash \exCons{u}{v}: \tyList{\tyInt}
}
%
\and
%
\inferrule*
{
   \Gamma \vdash v: \tyInt
}
{
   \Gamma \vdash \exConsProjL{v}: \tyListProj{\tyInt}
}
%
\and
%
\inferrule*
{
   \Gamma \vdash v: \tyList{\tyInt}
}
{
   \Gamma \vdash \exConsProjR{v}: \tyList{\tyInt}
}
\end{smathpar}
\caption{Typing rules for terms}
\end{figure}
